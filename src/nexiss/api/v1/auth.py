...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db_session)) -> TokenResponse:
    ...
    token = create_access_token(user_id=user.id, org_id=organization.org_id, expires_delta=expires_delta)
    return TokenResponse(access_token=token, expires_in_seconds=int(expires_delta.total_seconds()))
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db_session)) -> TokenResponse:
    ...
    if target_org_id not in memberships:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Requested org not available")

    expires_delta = timedelta(minutes=settings.jwt_access_token_expire_minutes)
    token = create_access_token(user_id=user.id, org_id=target_org_id, expires_delta=expires_delta)
    return TokenResponse(access_token=token, expires_in_seconds=int(expires_delta.total_seconds()))
...
async def switch_org(
    ...
    expires_delta = timedelta(minutes=settings.jwt_access_token_expire_minutes)
    token = create_access_token(user_id=auth.user.id, org_id=payload.org_id, expires_delta=expires_delta)
    return TokenResponse(access_token=token, expires_in_seconds=int(expires_delta.total_seconds()))
